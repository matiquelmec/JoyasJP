...
// Inicializar el cliente SQLite local usando el archivo local.db
const client = createClient({
  url: 'file:local.db',
});

// Importar los mocks iniciales para poblar la base de datos
...
async function main() {
  console.log('⚡ Inicializando base de datos SQLite local (local.db)...');
  ...
  `);
  console.log('✅ Tabla "configuration" creada o ya existente.');
  ...
  `);
  console.log('✅ Tabla "bundle_items" creada o ya existente.');
  ...
  `);
  console.log('✅ Tabla "coupons" creada o ya existente.');
  ...
  if (productRows.rows[0].count === 0) {
    console.log('📦 Insertando productos iniciales de demostración...');
    ...
    console.log(`🎉 Se insertaron ${initialProducts.length} productos iniciales con éxito.`);
  }

  console.log('\n🌟 Base de datos SQLite local inicializada de forma impecable.');
  client.close();
}

main().catch(err => {
  console.error('❌ Error durante la inicialización de la base de datos local:', err);
  process.exit(1);
});

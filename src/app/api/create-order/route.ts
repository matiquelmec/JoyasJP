...
export async function POST(request: Request) {
  try {
    ...
    if (!customer_name || !shipping_address || !contact_email || !ordered_products) {
    ...
    if (error) {
      ...
      if (error.message.includes('Insufficient stock')) {
        return NextResponse.json({ error: 'One or more products are out of stock.', details: error.message }, { status: 409 }); // 409 Conflict
      }
